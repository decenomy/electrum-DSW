...
class CoinGecko(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        exchange_rates = await self.get_json('api.coingecko.com', '/api/v3/exchange_rates')
        market_data = await self.get_json('api.coingecko.com', '/api/v3/coins/sapphire?localization=false&tickers=false&market_data=true&community_data=false&developer_data=false&sparkline=false')
        return dict([(ccy.upper(), Decimal(d['value']) * Decimal(market_data['market_data']['current_price']['btc']))
                     for ccy, d in exchange_rates['rates'].items()])
    ...
    async def request_history(self, ccy):
        history = await self.get_json('api.coingecko.com',
                                      '/api/v3/coins/sapphire/market_chart?vs_currency=%s&days=max' % ccy)
